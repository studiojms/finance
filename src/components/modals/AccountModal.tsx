...
import { X, Banknote, CreditCard, Wallet, Landmark, PiggyBank, Briefcase, Coins, Receipt, CreditCard as CardIcon } from 'lucide-react';
...
export function AccountModal({ isOpen, onClose, userId, editingAccount, transactions }: AccountModalProps) {
  ...
  const [initialBalance, setInitialBalance] = useState(editingAccount?.initialBalance?.toString() || editingAccount?.balance.toString() || '0');
  const [initialBalanceDate, setInitialBalanceDate] = useState(editingAccount?.initialBalanceDate || format(new Date(), 'yyyy-MM-dd'));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const accountTransactions = transactions.filter(t => 
        ...
        t.isConsolidated &&
        (isAfter(parseISO(t.date), parseISO(initialBalanceDate)) || isEqual(parseISO(t.date), parseISO(initialBalanceDate)))
      );
    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <h2 className="text-xl font-bold text-slate-800 flex-1 text-center">{editingAccount ? 'Editar Conta' : 'Nova Conta'}</h2>
          <button 
            onClick={handleSave}
            className="text-lg font-bold text-emerald-600 transition-colors"
          >
            Salvar
          </button>
        </div>
        <div className="space-y-4">
          <input type="text" placeholder="Nome da Conta" value={name} onChange={e => setName(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold" />
          <select value={type} onChange={e => setType(e.target.value as any)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold">
          ...
          <input type="number" placeholder="Saldo Atual" value={balance} onChange={e => setBalance(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold" />
          
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2">Saldo Inicial (Base)</label>
            <div className="flex gap-2">
              <input type="number" placeholder="Saldo Inicial" value={initialBalance} onChange={e => setInitialBalance(e.target.value)} className="flex-1 p-4 bg-slate-50 rounded-2xl font-bold" />
              <input type="date" value={initialBalanceDate} onChange={e => setInitialBalanceDate(e.target.value)} className="w-40 p-4 bg-slate-50 rounded-2xl font-bold text-sm" />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 px-2">
              {['#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#64748b'].map(c => (
                <button key={c} onClick={() => setColor(c)} className={cn("w-8 h-8 rounded-full border-2", color === c ? "border-slate-800" : "border-transparent")} style={{ backgroundColor: c }} />
              ))}
            </div>
          </div>
